...
const state = {
  ...
  function renderUsers() {
    ...
    if (state.users.length === 0) {
      tbody.innerHTML = `<tr><td colspan="5"><div class="empty-state"><div class="es-icon">👤</div><div class="es-title">No users yet</div><div class="es-sub">Click "Add User" to get started</div></div></td></tr>`;
      return;
    }
    tbody.innerHTML = state.users.map(u => `
      <tr>
        ...
        <td>${u.email}</td>
        <!-- <td><span class="badge ${u.role==='Admin'?'badge-red':u.role==='Volunteer'?'badge-amber':'badge-green'}">${u.role}</span></td> -->
        <td>${u.joined}</td>
        <td class="td-actions">
          <button class="action-btn edit" onclick="editUser(${u.id})" title="Edit">✏️</button>
          <button class="action-btn del" onclick="deleteUser(${u.id})" title="Delete">🗑️</button>
        </td>
      </tr>`).join('');
  }
